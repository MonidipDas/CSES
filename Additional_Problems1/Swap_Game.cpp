...
void solve() {
    //edge with horizontally and vertically connected elements.
    //cout<<(9*8*7*6*5*4*3*2*1)<<endl;
    //362880
    //bfs on (strings) as form of state-graph
    //operations like flip,swap we may use bfs to find minimum no of moves
    ...
    for(int r=0;r<3;r++){
        for(int c=0;c<3;c++){
            ...
            //c->0,1 connect to 1,2 respctvly for all rows like this
            if(c<2){
                edges.pb({id,id+1});
            }
            //r->0,1 connect to 3,4 respctvly for all rows like this
            if(r<2){
                edges.pb({id,id+3});
            }
        }
