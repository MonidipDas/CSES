...
void solve() {
    ...
    //diff array can,t be made as constraint here is 1e9
    //we have to use sweep-line technique here
    //for[a,b] we do diff[a]+=1 and diff[b]-=1 here b is not inclusive
    //and we take pref sum over diff array
    //in sweep line we store it as {a,1} and {b,-1}
    ...
    for(auto [t,c]:events){
        curr+=c;                //pref[i]
        ans=max(curr,ans);      //ans=max(ans,pref[i])
    }
